...
async def list_holidays(current_user: CurrentUser, db: DBSession, year: Optional[int] = None):
    ...
    if year:
        result = await db.execute(
            text("SELECT * FROM master.payroll_holidays WHERE EXTRACT(YEAR FROM holiday_date) = :y ORDER BY holiday_date"),
            {"y": year}
        )
    else:
        result = await db.execute(
            text("SELECT * FROM master.payroll_holidays ORDER BY holiday_date")
        )
...
async def add_holiday(body: HolidayIn, current_user: CurrentUser, db: DBSession):
    ...
    result = await db.execute(
        text("""
            ...
            VALUES (:holiday_date, :holiday_name, :description)
...
async def _ensure_tables(db: DBSession):
    ...
    await db.execute(text("""
        CREATE TABLE IF NOT EXISTS master.payroll_config (
            ...
            week_off_days VARCHAR(20) DEFAULT '0',
