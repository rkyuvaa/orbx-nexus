...
// ── EB Consumption ──
export function EBConsumptionReport() {
  const { activeFY } = useAuthStore();
  const [fromDate, setFromDate] = useState(firstOfMonth);
  const [toDate, setToDate] = useState(today);
  const [enabled, setEnabled] = useState(false);
  ...
  const { data } = useQuery<any>({
    queryKey: ["report-eb", activeFY, fromDate, toDate, enabled],
    queryFn: async () => (await api.get(`/reports/eb-consumption?fy=${activeFY}&from_date=${fromDate}&to_date=${toDate}`)).data,
    enabled,
  });
  ...
  const handlePrint = () => {
    ...
    entries.forEach((r: any) => {
      rowsHtml += `<tr>
        <td style="text-align: center; white-space: nowrap;">${r.reading_date}</td>
        <td style="text-align: center;">${r.meter_no}</td>
        <td style="text-align: right;">${r.previous_reading}</td>
        <td style="text-align: right;">${r.current_reading}</td>
        <td style="text-align: right;">${r.units_consumed}</td>
        <td style="text-align: right;">${r.rate_per_unit}</td>
        <td style="text-align: right;">₹${formatAmount(r.amount)}</td>
      </tr>`;
    });
        ...
        <body>
          ...
          <table class="items-table">
            <thead>
              <tr>
                <th style="width: 85px; text-align: center;">Date</th>
                <th style="width: 80px; text-align: center;">Meter No.</th>
                <th style="width: 90px; text-align: right;">Prev Reading</th>
                <th style="width: 90px; text-align: right;">Curr Reading</th>
                <th style="width: 65px; text-align: right;">Units</th>
                <th style="width: 70px; text-align: right;">Rate/Unit</th>
                <th style="width: 90px; text-align: right;">Amount</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr class="totals-row">
                <td colspan="4" style="text-align: right; text-transform: uppercase;">Total</td>
                <td style="text-align: right;">${data.total_units}</td>
                <td></td>
                <td style="text-align: right;">₹${formatAmount(data.total_amount)}</td>
              </tr>
            </tbody>
          </table>
          <div class="footer-info" style="margin-top: 30px; border-top: 1px solid #ddd; padding-top: 8px; display: flex; justify-content: space-between; font-size: 10px; color: #777;">
  ...
  return (
    <Box>
      <Box className="no-print">
        <PageHeader title="EB Consumption" breadcrumbs={[{ label: "Reports" }, { label: "EB Consumption" }]} />
        <FilterRow>
          <TextField label="From" type="date" size="small" value={fromDate} onChange={(e) => setFromDate(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
          <TextField label="To" type="date" size="small" value={toDate} onChange={(e) => setToDate(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
          <Button variant="contained" startIcon={<Search />} onClick={() => setEnabled(true)}>Show</Button>
          {entries.length > 0 && (
            <>
              <StatPill label="Total Units" value={`${data.total_units} kWh`} color="#1976d2" />
              <StatPill label="Total Amount" value={`₹${formatAmount(data.total_amount)}`} color="#ed6c02" />
              <Button variant="outlined" size="small" startIcon={<Print />} onClick={handlePrint}>Print</Button>
            </>
          )}
        </FilterRow>
      ...
      {entries.length > 0 && (
        <Paper variant="outlined" sx={{ overflow: "auto" }}>
          <PrintTable
            ...
            columns={["Date", "Meter No.", "Prev Reading", "Curr Reading", "Units", "Rate/Unit", "Amount"]}
            rows={entries.map((r: any) => [r.reading_date, r.meter_no, r.previous_reading, r.current_reading, r.units_consumed, r.rate_per_unit, `₹${formatAmount(r.amount)}`])}
            totals={["", "", "", "", `${data.total_units}`, "", `₹${formatAmount(data.total_amount)}`]}
          />
        </Paper>
      )}
      {entries.length === 0 && enabled && (
        <Paper variant="outlined" sx={{ p: 3, textAlign: "center" }}>
          <Typography color="text.secondary">No EB consumption data found for the selected date range.</Typography>
        </Paper>
      )}
    </Box>
  );
